...
# Select how to update and use the model 
# 'load_train' to load and train model further
# 'load' to load model  and use for prediction
# 'train' to train model from scratch and save it
...
# Function to freeze layer in model 
...
model.summary()
plot_model(model, show_shapes=True, rankdir='TB', to_file='model.png')

## Get data images and steering angles
sideOffset = 0.2
batchSize = 16
csvFolder = 'TrainingData_Large'
...
if loadModel == 'load_train':
    model = load_model(modelFile, custom_objects={'tf': tf, 'ResizeNormalizeLambda': ResizeNormalizeLambda})
    model_cropInput = load_model('model_cropinput.h5')

if (loadModel == 'train') or (loadModel == 'load_train'):
    ...
    plt.savefig('examples/TrainHistory.png', bbox_inches='tight')
    plt.show()
elif loadModel == 'load':
    model = load_model(modelFile, custom_objects={'tf': tf, 'ResizeNormalizeLambda': ResizeNormalizeLambda})
    model_cropInput = load_model('model_cropinput.h5')
...
imgCropped = np.uint8(model_cropInput.predict(xval))
...
for n, ax in enumerate(axs):
    ax[0].imshow(xval[n])
    ax[1].imshow(imgCropped[n])

plt.tight_layout()
plt.savefig('examples/croppedImage.png', bbox_inches='tight')
plt.show()
...
print()
print('[Estimated angle | Real angle]\n',np.column_stack((steer[0:8],yval[0:8])))

# Plot histogram (future update)
